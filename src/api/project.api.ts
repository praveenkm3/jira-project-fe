...
export async function addProjectMembers(projectId: string, data: string[]) {
  const response = await api.post(`/api/project/${projectId}/members`, {
    data,
  });
  return response.data;
}
...
export async function addProjectStatuses(
  projectId: string,
  status_name: string,
) {
  const response = await api.post(`/api/project/add-status/${projectId}`, {
    status_name,
  });
  return response.data;
}
