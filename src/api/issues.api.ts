...
export async function fetchProjectIssues(
  ...
  const response = await api.get(
    `/api/issues/${projectId}?pageNumber=${pageNumber}&pageRecords=${pageRecords}&field=${field}&value=${value}`,
  );
  return response.data;
}
...
export async function updateIssueStatus(issueId: string, status: string) {
  const response = await api.patch(`/api/issues/${issueId}/change-status`, {
    status,
  });
  return response.data;
}
